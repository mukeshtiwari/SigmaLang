...
| A15 | The design-time result reaches compiled statements, claim included, across the whole source language | `Compiler/DslInstantiate.v`, axiom-free: `live_claim_is_design_time` (a branch's computed claim equals the syntactic one under a faithful environment) and `compiled_statement_leaves_determine` (`SEqs`/`SAnd`/`SOr`/`SThresh`: a checked statement compiles to a tree whose every leaf determines its live claim) | supported |
| A15 | The gap of A6 is **not reachable** in the Swiss Post protocol, and what prevents it is an unstated convention rather than a check | System Specification: every exponentiation-proof call site passes a base vector led by the generator — `(g, c_pCC,id)`, `(g, c_ck,id)`, `(g, γ1, φ1,0)`, `(g, γ2)` — and the generator is 2 or 3 by construction, so the all-identity vector cannot arise. The two plaintext-equality sites use `EL_pk,0` and `∏ pk_CCR,i`, election-level keys fixed at setup under `VerifyKeyGenerationSchnorrProofs`; both being the identity needs the tally components to collude, and they hold the decryption key already | supported — a NEGATIVE result on exploitability, and the specification point of A6 stands |
| A16 | Three independent Σ-protocol libraries accept a relation that does not determine its witness | `Evidence/libraries/`: sigma-rs `3e12c83` and zkp 0.8.0 (`cdca489`) demonstrated by executable tests in their own harnesses; zksk (`075ccdc`) by reading `DLRep.__init__`, whose only base check is same-group. The relation is the two-attribute credential on one generator | supported — NOT a vulnerability in any deployment, and the evidence README says so |
| A17 | Belenios §4.15 is a deployed statement where the visible verdict and the semantic truth differ, and the gap is closed by a hand check | The relation `1 = alpha^u g^v`, `A0 = beta^u y^v` has four distinct base names, so the incidence system forces `u = v = 0` and the checker reports determined. But `alpha = g^r` and `beta = y^r g^m`, so eliminating `v` leaves `A0 = g^(m u)`: when `m = 0` the true kernel is a line. The verifier's first step, `A0 != 1`, is unsatisfiable exactly then | supported — this is our own criterion's limitation exhibited in the field, and it is evidence FOR the completeness theorem, not against it |
| A18 | Every relation of the six specifications establishes exactly the secrets it declares | `Executable/Analyse` prints the established forms for all ten transcribed relations; each is the list of its own secrets, individually | supported |
...
- "Belenios §4.15 is broken" — A17 is our criterion's blind spot, not a defect in Belenios; their verifier closes it, deliberately and in the right place
- "N libraries are vulnerable" — A16 is about APIs accepting a degenerate statement, not about any deployed system being exploitable; zksk in particular was read, not run
- "Swiss Post's exponentiation proof is broken" / "we found a reachable flaw" — A15 settles it: the callers never reach the gap, because every base vector is led by the generator. A6 is a specification-hygiene point, not a vulnerability
- "we audited the Swiss Post system" — we read its System Specification's call sites, not its implementation or deployment
