...
for t in $THEORIES; do
  ...
  if [ -d "$src" ]; then
    cp -R "$src" "docs/$t"
    chmod -R u+w "docs/$t"
    # rocqdoc's stock stylesheet paints every documentation block with a
    # solid blue background, which is unreadable now that these sources
    # are mostly prose.  Replace it with ours.
    cp etc/coqdoc.css "docs/$t/coqdoc.css"
...
<li><a href="Examples/toc.html"><span class="name">Examples</span></a>
    <span class="desc">— a complete instance over a concrete group,
    extracted to OCaml.</span></li>
</ul>
