...
Section LinearRelation.
  ...
  Section Proofs.
    ...
    Section Instances.
      ...
      (* Pedersen opening (m = 1, n = 2): ∃ v r, C = g^v · h^r.
         Same shape as Okamoto with the bases renamed. *)
      ...
      (* AND of two independent Schnorr statements (m = n = 2):
         ∃ x₁ x₂, h₁ = g₁^x₁ ∧ h₂ = g₂^x₂.
         A witness that does not occur in an equation shows up as a
         gid entry in that row. *)
      ...
      (* Equality of Pedersen-committed values (m = 2, n = 3):
         ∃ v r₁ r₂, C₁ = g^v · h^r₁ ∧ C₂ = g^v · h^r₂.
         The shared witness v occupies the same column in both rows,
         while each randomness has its own column. *)
      ...
      (* Diffie-Hellman tuple (m = 3, n = 2):
         ∃ a b, h₁ = g^a ∧ h₂ = g^b ∧ h₃ = h₁^b,
         i.e. (g, h₁, h₂, h₃) = (g, g^a, g^b, g^{ab}).  The public
         element h₁ is itself used as a base in the last row. *)
      ...
      (* m-fold Chaum-Pedersen / DLEQ (m arbitrary, n = 1):
         ∃ x, ∀ i, hs[i] = gs[i]^x.
         The matrix is the column vector of bases gs. *)
